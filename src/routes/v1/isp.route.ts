...
// This route gets the ISP of an IP address
router.get("/v1/ip/isp/:ip", async (req: Request, res: Response) => {
    ...
    try {
        ...
        Logger.log("cyan", "Response", `IP: ${ip} | ISP: ${response.traits.isp ?? "Unknown"}`); // Log the response

        res.status(200).json({ isp: response.traits.isp ?? "Unknown" }); // Return the ISP
    } catch (e) {
        Logger.log("red", "Error", `Error while fetching ISP of ${ip}: ${String(e)}`); // Log the error
