...
// This route gets the country of an IP address
router.get("/v1/ip/country/:ip", async (req: Request, res: Response) => {
    const ip = req.params.ip; // Get the IP address from the request parameters
    ...
    try {
        ...
        Logger.log("red", "Error", `Error while fetching country of ${ip}: ${String(e)}`); // Log the error
