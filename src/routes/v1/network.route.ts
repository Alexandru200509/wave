...
// This route gets the network of an IP address
router.get("/v1/ip/network/:ip", async (req: Request, res: Response) => {
    ...
    try {
        ...
        Logger.log("cyan", "Response", `IP: ${ip} | Network: ${response.traits.network ?? "Unknown"}`); // Log the response

        res.status(200).json({ network: response.traits.network ?? "Unknown" }); // Return the network
    } catch (e) {
